...
class RelatoriosController:
    ...
    def abrir_tela(self):
        while True:
            ...
            if opcao == 0: break
            elif opcao == 1: self.relatorio_clinica()
            elif opcao == 2: self.relatorio_paciente()
            elif opcao == 3: self.relatorio_profissional()

    def formata_atendimento(self, at):
    ...
    def relatorio_clinica(self):
        ...
        for at in self.__atendimento_DAO.get_all():
            if at.clinica.nome == nome:
                relatorio += self.formata_atendimento(at)
                achou = True
        if not achou:
